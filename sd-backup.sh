...
declare TS CLI_param_action CLI_param_doNotAskForConfirmation _disk disk _imgFile imgFile
...
disk="$1"
if [[ ! "$@" =~ ^/dev/r{0.1}disk[0-9]+ ]]; then disk=''; fi
...
if [[ "$@" =~ "-".*[ry].* || "$@" =~ "--yes" || "$@" =~ "--restore"  ]]; then imgFile="${defaultImgFile}"; fi
_imgFile="${imgFile##*/}"
